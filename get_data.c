...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/11/15 18:48:35 by bpole             #+#    #+#             */
/*   Updated: 2019/11/15 19:37:33 by bpole            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_data		*ft_get_values(t_data *data, unsigned flag,
...
static char			*ft_get_filename(t_data *data)
{
	...
	if (!(buff = ft_memalloc(sizeof(char) * (ft_strlen(data->dir) + 40))))
		return (NULL);
					...
					int level, t_width *width)
{
	...
	errno = 0;
	if (!(d = opendir(data->dir)) && S_ISDIR(data->st_mode)
			&& (!level || flag & FLAG_R_UP))
		ft_fprintf(2, "ls: %s: %s\n", data->filename, strerror(errno));
	fill_width(data, width);
	if (!is_hidden(data->filename, data->flag) || !data->level)
		data = ft_get_values(data, flag, level, width);
	if (d)
		closedir(d);
	return (data);
}
