...
static int     ft_creat_data(t_avltree1	**root, int ac, char **av, unsigned flag)
{
    t_data	*data;
    int     i;
    t_width width;
...
{
    if (S_ISLNK((data->stats).st_mode) && *flag &FLAG_R_UP && !ft_check_link(data))
        return;
    if ((S_ISDIR((data->stats).st_mode) && (!is_hidden(data->filename, data->flag)
